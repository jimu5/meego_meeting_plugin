...
const ScheduleTable = ({ disabled, SDKReady }: IScheduleTable) => {
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      render: (text, record: WorkItemMeeting, index) => {
        return (
          <TextRender text={record.calendar_event_organizer?.display_name} />
        );
      },
    },
    {
    ...
    },
    {
        ...
        render: (text, record, index) => {
          return (
              <TimeRender startTime={record?.meeting_time?.start_time} endTime={record?.meeting_time?.end_time} />
          );
        },
    },
    {
      ...
      render: (text, record, index) => {
        return (
            <LinkRender link={record.meeting_record_url} type={LinkType.MIAOJI} />
        );
      },
    },
    {
      ...
      render: (text, record, index) => {
        return (
          <StatusRender status={record?.meeting_status} />
        );
      },
    },
    {
    ...
    },
    {
      ...
      render: (text, record: WorkItemMeeting, index) => {
        return (
          <UserProfileComp avatar={record.bind_operator_info?.avatar_url} name={record?.bind_operator_info?.name_cn} userId={record.bind_operator_info?.meego_user_key} />
        );
      },
    },
    {
      ...
      render: (text, record: WorkItemMeeting, index) => {
        return (
          <ActionRender meetingId={record?.meeting_id} isException={Boolean(record?.calendar_event_recurrence)} calendarId={record.calendar_id} calendarEventId={record.calendar_event_id} />
        );
      },
    },
  ], []);
